    ...
    try:
        ...
        with concurrent.futures.ProcessPoolExecutor(num_processes) as executor:
            with open(input_path, "rb") as f: # Move to the end of the file to ensure it's accessible
                boundaries = find_chunk_boundaries(f, 40 * num_processes, b"<|endoftext|>")
            
            print(f"Length of chunks: {len(boundaries)}")
            compiled_special_tokens_re = re.compile("|".join([re.escape(st) for st in special_tokens]))
            PAT = r"""'(?:[sdmt]|ll|ve|re)| ?\p{L}+| ?\p{N}+| ?[^\s\p{L}\p{N}]+|\s+(?!\S)|\s+"""
            compiled_words_re = re.compile(PAT)
            futures = [executor.submit(read_file_in_chunks, input_path, start, end, compiled_special_tokens_re, compiled_words_re) for start, end in zip(boundaries[:-1], boundaries[1:])]
            i = 1
            for future in concurrent.futures.as_completed(futures):
                result = future.result()
                pretokenized_counter.update(result)
                print(f"\rRead and pretokenize iteration: {i}, Length of pretokenized words counter: {len(pretokenized_counter)}", end="", flush=True)
                i += 1
            
            print(f"\nLength of pretokenized words counter: {len(pretokenized_counter)}")
...
def read_file_in_chunks(
    ...
    compiled_special_tokens_re: re.Pattern[str],
    compiled_words_re: re.Pattern[str]
) -> Counter[str]:
    try:
        counter = Counter()
        ...
        with open(input_path, "rb") as f:
            ...
            for content in compiled_special_tokens_re.split(chunk):
                if content.strip() == "":
                    continue

                counter = build_pretokenized_counter(content, counter, compiled_words_re)
        return counter
    except Exception as e:
...
def compute_merges(
    ...
    for i in range(vocab_size - len(vocabulary)):
        ...
        print(f"\rMerge iteration: {i}", end="", flush=True)
